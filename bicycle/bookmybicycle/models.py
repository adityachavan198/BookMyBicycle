...
class Stand(models.Model):
    ...
    sloc= models.CharField(max_length=2)

    def __str__(self):
        return f"sid ={self.sid}, sstate={self.sstate}, scity={self.scity}, sloc={self.sloc}"

class Cycle(models.Model):
    cid= models.AutoField(primary_key= True)
    cstate= models.BooleanField(default= False)
    sid= models.ForeignKey(Stand, on_delete= models.CASCADE)
    rides= models.ManyToManyField(get_user_model(), blank=True)


    def __str__(self):
        return f"cid = {self.cid}, sid = {self.sid}"

class Book(models.Model):
    bid= models.AutoField(primary_key=True)
    cid= models.ForeignKey(Cycle, on_delete= models.CASCADE)
    sid_start= models.ForeignKey(Stand, on_delete= models.CASCADE, related_name="sid_start",default= -1)
    sid_end= models.ForeignKey(Stand, on_delete= models.CASCADE, related_name="sid_end",default= -1)
    uid= models.ForeignKey(UserOfApp, on_delete=models.CASCADE)
#    from_date_time =
#    to_date_time = 
